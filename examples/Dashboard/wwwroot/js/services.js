...
export async function refreshServicePanels() {
  ...
  host.innerHTML = '';
  const defs = [
    { title: 'OrderService', service: 'order' },
    { title: 'InventoryService', service: 'inventory' },
    { title: 'NotificationService', service: 'notification' },
  ];
  for (const d of defs) {
    ...
    try {
      const data = await fetchControlState(d.service);
      if (d.service === 'inventory' && data.toggles?.[0]?.mode)
      ...
      for (const t of data.toggles ?? []) {
        ...
        btnApply.addEventListener('click', async () => {
          try {
            ...
            const body =
              m === 'succeed-after'
                ? { mode: 'succeed-after', failureCount: fc }
                : { mode: m };
            await setToggle(d.service, t.key, body);
        ...
        btnCycle.addEventListener('click', async () => {
          try {
            await setToggle(d.service, t.key, {});
            await refreshServicePanels();
          } catch (e) {
            if (errEl) errEl.textContent = e.message;
          }
