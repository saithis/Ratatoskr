...
var consumerCs =
    builder.Configuration.GetConnectionString("consumerdb")
    ?? throw new InvalidOperationException("Connection string 'consumerdb' is not configured.");
var playgroundCs =
    builder.Configuration.GetConnectionString("playgrounddb")
        ...
        return TypedResults.Ok(
            new
            {
                ...
                stores = new[]
                {
                    ...
                    {
                        key = "publisher",
                        contextName = "PublisherDbContext",
                        role = "Outbox + inbox (publisher-side orders)",
                    },
                    new
                    {
                        key = "consumer",
                        contextName = "ConsumerDbContext",
                        role = "Command inbox + outcome outbox",
                    },
                },
            }
        );
...
app.MapGet(
        "/api/playground/rabbit-depths",
        async Task<IResult> (HttpContext http) =>
        {
            ...
            foreach (var (slug, q) in PlaygroundScenarioManifest.EnumerateRabbitDepthProbeTargets())
            {
                ...
                var retry = await SafeMessageCountAsync(
                    channel,
                    PlaygroundAmqpNames.RetryQueueName(main),
                    cancellationToken
                );
                var dlq = await SafeMessageCountAsync(
                    channel,
                    PlaygroundAmqpNames.DlqQueueName(main),
                    cancellationToken
                );
                queues.Add(
                    new
...
app.MapGet(
    "/api/orders",
    async ([FromServices] PublisherDbContext db) =>
    {
        var orders = await db
...
playground.MapGet(
    ...
    {
        var publisherOutboxPoisoned = await PlaygroundSqlMetrics.CountPoisonedOutboxAsync(
            publisherDb,
            cancellationToken
        );
        var publisherInboxPoisoned = await PlaygroundSqlMetrics.CountPoisonedInboxAsync(
            publisherDb,
            cancellationToken
        );
        var consumerInboxPoisoned = await PlaygroundSqlMetrics.CountPoisonedInboxAsync(
            consumerDb,
            cancellationToken
        );
        return TypedResults.Ok(
            new
            {
                publisher = new
                {
                    outboxPoisoned = publisherOutboxPoisoned,
                    inboxPoisoned = publisherInboxPoisoned,
                },
...
playground.MapGet(
    "/scenarios",
    ([FromServices] ScenarioRunService runs) => TypedResults.Ok(runs.ListCatalog())
);
...
playground.MapGet(
    "/runs/{runId:guid}",
    async Task<IResult> (HttpContext http, Guid runId, [FromServices] ScenarioRunService runs) =>
    {
