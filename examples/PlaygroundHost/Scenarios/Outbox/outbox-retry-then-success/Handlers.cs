...
public sealed class OutboxRetryThenSuccessOrderFailedHandler(PublisherDbContext db, TimeProvider time, ILogger<OutboxRetryThenSuccessOrderFailedHandler> logger)
    : IMessageHandler<OutboxRetryThenSuccessOrderFailed>
{
    public async Task HandleAsync(OutboxRetryThenSuccessOrderFailed message, MessageProperties properties, CancellationToken cancellationToken)
    {
        var order = await db.Orders.FirstOrDefaultAsync(o => o.Id == Guid.Parse(message.OrderId), cancellationToken);
        if (order is null) return;
        var now = time.GetUtcNow().UtcDateTime;
...
public sealed class OutboxRetryThenSuccessOrderFulfilledNotifyHandler(ILogger<OutboxRetryThenSuccessOrderFulfilledNotifyHandler> logger) : IMessageHandler<OutboxRetryThenSuccessOrderFulfilled>
{
    public Task HandleAsync(OutboxRetryThenSuccessOrderFulfilled message, MessageProperties properties, CancellationToken cancellationToken)
    {
