...
public class EfCoreTransportTests(RabbitMqContainerFixture rabbitMq, PostgresContainerFixture postgres)
    : InboxTestBase(rabbitMq, postgres)
{
    ...
    public async Task PublishDirectAsync_BasicFlow_MessageDeliveredViaInbox()
    {
        ...
        await StartTestAsync(services =>
        {
            ...
            {
                bus.AddEventPublishChannel("efcore-events", c => c.WithEfCore().Produces<TestEvent>());
                bus.AddEventConsumeChannel("efcore-events", c => c
        ...
        await StartTestAsync(services =>
        {
            services.AddRatatoskr(bus =>
            {
                ...
                bus.AddEventConsumeChannel("channel-b", c => c
                    .Consumes<TestEvent>(m => m.WithHandler<InboxHandlerB>("handler-b"))
                    .UseInbox<TestDbContext>());
                bus.AddEfCoreDurability<TestDbContext>(d => d.UseInbox());
            });
    ...
    public async Task PublishDirectAsync_TrackingStages_PublishedAndSentFire()
    {
        ...
        await StartTestAsync(services =>
        {
            services.AddRatatoskr(bus =>
            {
                bus.AddEventPublishChannel("track-events", c => c.WithEfCore().Produces<TestEvent>());
                bus.AddEventConsumeChannel("track-events", c => c
                    .Consumes<TestEvent>(m => m.WithHandler<InboxHandlerA>("handler-a"))
                    .UseInbox<TestDbContext>());
                bus.AddEfCoreDurability<TestDbContext>(d => d.UseInbox());
            });
    ...
    public void PublishDirectAsync_ChannelWithoutInbox_ThrowsAtStartup()
    {
        ...
        var act = () => new ServiceCollection().AddRatatoskr(bus =>
        {
        ...
        act.Should().Throw<InvalidOperationException>()
            .WithMessage("*EF Core transport*UseInbox*");
    }
    ...
    public async Task EfCoreTransport_SendActivity_Created()
    {
        ...
        await StartTestAsync(services =>
        {
            services.AddRatatoskr(bus =>
            {
                bus.AddEventPublishChannel("otel-efcore", c => c.WithEfCore().Produces<TestEvent>());
                bus.AddEventConsumeChannel("otel-efcore", c => c
                    .Consumes<TestEvent>(m => m.WithHandler<InboxHandlerA>("handler-a"))
                    .UseInbox<TestDbContext>());
                bus.AddEfCoreDurability<TestDbContext>(d => d.UseInbox());
            });
        ...
        await WaitForConditionAsync(
            () => Task.FromResult(activities.Any(a =>
                a.OperationName == "send efcore"
                && a.TagObjects.Any(t => t.Key == "messaging.message.id" && (string?)t.Value == "msg-otel-1"))),
        ...
        sendActivity.TagObjects.Should().Contain(t => t.Key == "messaging.system" && (string?)t.Value == "efcore");
        sendActivity.TagObjects.Should().Contain(t => t.Key == "messaging.operation.name" && (string?)t.Value == "send");
        sendActivity.TagObjects.Should().Contain(t => t.Key == "messaging.operation.type" && (string?)t.Value == "send");
        sendActivity.TagObjects.Should().Contain(t => t.Key == "messaging.message.id" && (string?)t.Value == "msg-otel-1");
    ...
    public async Task EfCoreTransport_Metrics_Recorded()
    {
        ...
        await StartTestAsync(services =>
        {
            services.AddRatatoskr(bus =>
            {
                bus.AddEventPublishChannel("metrics-efcore", c => c.WithEfCore().Produces<TestEvent>());
                bus.AddEventConsumeChannel("metrics-efcore", c => c
                    .Consumes<TestEvent>(m => m.WithHandler<InboxHandlerA>("handler-a"))
                    .UseInbox<TestDbContext>());
                bus.AddEfCoreDurability<TestDbContext>(d => d.UseInbox());
            });
        ...
        await WaitForConditionAsync(
            () => Task.FromResult(metricMeasurements.Any(m =>
