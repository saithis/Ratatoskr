...
public class EfCoreMetricsBackgroundServiceTests
{
    ...
    public async Task UpdateMetricsAsync_AccuratelyCountsPendingAndPoisonedMessages()
    {
        ...
        await using (var seedScope = provider.CreateAsyncScope())
        {
            ...
            // Add Outbox Messages
            // 2 pending
            var outPending1 = OutboxMessageEntity.Create([1, 2, 3], new MessageProperties { Id = Guid.NewGuid().ToString() }, timeProvider, "test");
            var outPending2 = OutboxMessageEntity.Create([1, 2, 3], new MessageProperties { Id = Guid.NewGuid().ToString() }, timeProvider, "test");
            // 1 poisoned
            var outPoisoned = OutboxMessageEntity.Create([1, 2, 3], new MessageProperties { Id = Guid.NewGuid().ToString() }, timeProvider, "test");
            outPoisoned.MarkAsPoisoned("error", timeProvider);
            // 1 processed (should not be counted)
            var outProcessed = OutboxMessageEntity.Create([1, 2, 3], new MessageProperties { Id = Guid.NewGuid().ToString() }, timeProvider, "test");
            outProcessed.MarkAsProcessed(timeProvider);

            dbContext.Set<OutboxMessageEntity>().AddRange(outPending1, outPending2, outPoisoned, outProcessed);

            // Add Inbox Statuses
            // 3 pending
            var inPending1 = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler1", timeProvider);
            var inPending2 = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler2", timeProvider);
            var inPending3 = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler1", timeProvider);
            // 2 poisoned
            var inPoisoned1 = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler1", timeProvider);
            inPoisoned1.MarkAsPoisoned("error", timeProvider);
            var inPoisoned2 = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler2", timeProvider);
            inPoisoned2.MarkAsPoisoned("error", timeProvider);
            // 1 completed (should not be counted)
            var inCompleted = InboxHandlerStatusEntity.Create(Guid.NewGuid().ToString(), "handler1", timeProvider);
            inCompleted.MarkAsCompleted(timeProvider);

            dbContext.Set<InboxHandlerStatusEntity>().AddRange(inPending1, inPending2, inPending3, inPoisoned1, inPoisoned2, inCompleted);
            await dbContext.SaveChangesAsync();
        }

        var backgroundService = provider.GetServices<IHostedService>()
